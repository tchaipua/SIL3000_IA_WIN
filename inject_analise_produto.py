...
# 1. Cria todo o bloco de código da Classe Nova
nova_classe = """
# =====================================================================
...
class AnaliseProdutoWindow(ctk.CTkToplevel):
    def __init__(self, parent, config):
        ...
        self.btn_copy_id = ctk.CTkButton(self.frame_id, text="📋", width=20, height=20, fg_color="transparent", hover_color="#E0E0E0", text_color="black", command=lambda: self.clipboard_clear() or self.clipboard_append("CFG_PRODS"))
    ...
    def carregar_anos(self):
        # Puxa anos do crmov2 para o combobox
    ...
    def carregar_dados(self):
        ...
        try:
            ...
            where_clauses = ["1=1"]
            if ano != "Todos":
                where_clauses.append(f"YEAR(c2.CRMov2DAte) = {int(ano)}")
            if mes != "Todos":
                where_clauses.append(f"MONTH(c2.CRMov2DAte) = {int(mes)}")
...
if placeholder in full_content:
    print("Injetando nova classe no final do arquivo...")
    # Achamos o index do último bloco antes do if name main
    idx = full_content.rfind("if __name__ == '__main__':")
    fixed_content = full_content[:idx] + nova_classe + "\n\n" + full_content[idx:]
    
    # 2. Insere gatilho de abertura no MainHub
    abrir_metodo = """    def abrir_analise_produto(self):
        AnaliseProdutoWindow(self, self.config)"""
    
    # Injetando o método abrir_analise_produto no MainHub (linha 1040)
    idx_mainhub = fixed_content.rfind("def abrir_analise(self):")
    # Subir algumas linhas pra ser inserido no MainHub
    fixed_content = fixed_content.replace("    def abrir_analise(self):", abrir_metodo + "\n\n    def abrir_analise(self):")
    
    # 3. Conecta o comando no botão btn_analise_prod
    fixed_content = fixed_content.replace('command=lambda: None', 'command=self.abrir_analise_produto')

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(fixed_content)

print("Nova Classe AnaliseProdutoWindow acoplada com sucesso!")
